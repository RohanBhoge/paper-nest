import React, { useEffect, useState, useCallback, useRef } from "react";
import axios from "axios";

// --- API Endpoints ---
const API_BASE_URL = "http://localhost:5000/api/paperdata/paper";
// 🚨 ASSUMED ENDPOINT for OCR/Extraction
const OCR_API_URL = "http://localhost:5000/api/ocrTest/ocr-extract";

// --- Reusable Input Field Component (Supports editing/manual input) ---
const InputField = ({
  label,
  value,
  onChange,
  disabled = false,
  isReadonly = false,
  name,
}) => (
  <div>
    <label className="block text-slate-700 font-medium mb-1 text-sm">
      {label}
    </label>
    <input
      type="text"
      value={value}
      onChange={onChange}
      disabled={disabled}
      readOnly={isReadonly}
      name={name}
      className={`w-full px-3 py-2 border rounded-lg text-sm ${
        disabled || isReadonly
          ? "bg-slate-100 text-slate-600"
          : "bg-white border-slate-300 focus:border-blue-500"
      }`}
    />
  </div>
);

// --------------------------------------------------------------------------------------------------
// NEW INTEGRATED OMR COMPONENT
// --------------------------------------------------------------------------------------------------

const OmrIntegratedPage = ({ onBack, onSaved }) => {
  const [form, setForm] = useState({
    paperId: "",
    examName: "",
    className: "",
    subjectName: "",
    examDate: "",
    totalMarks: "",
  });

  const [files, setFiles] = useState([]);
  const [message, setMessage] = useState(null);

  const [fetchedData, setFetchedData] = useState(null);
  const [isLoading, setIsLoading] = useState(false);

  const fileInputRef = useRef(null);

  // --- Change Handler for Manual Input ---
  const handleFormChange = (e) => {
    const { name, value } = e.target;

    setForm((prev) => ({ ...prev, [name]: value }));

    // Clear fetched data if a critical field (like ID) changes manually
    if (name === "paperId") {
      setFetchedData(null);
    }
  };

  // --- DB Fetch Utility (GET) ---
  const handleFetchPaper = useCallback(async (paperId) => {
    if (!paperId) return;

    setIsLoading(true);
    setFetchedData(null);
    setMessage({
      type: "info",
      text: `Fetching full content for ${paperId}...`,
    });

    const url = `${API_BASE_URL}/${paperId}`;

    try {
      const response = await axios.get(url);

      if (response.data.success && response.data.data) {
        const data = response.data.data;
        setFetchedData(data);

        // Update form state with fetched metadata
        setForm((prevForm) => ({
          ...prevForm,
          paperId: data.paper_id,
          examName: data.exam_name || prevForm.examName,
          className: data.class || prevForm.className,
          subjectName: data.subject || prevForm.subjectName,
          examDate: data.exam_date
            ? new Date(data.exam_date).toLocaleDateString()
            : prevForm.examDate,
          totalMarks: data.marks || prevForm.totalMarks,
        }));

        setMessage({
          type: "success",
          text: `Details loaded for ${paperId}. Full content ready.`,
        });
      } else {
        setMessage({
          type: "warning",
          text: `Paper ID ${paperId} not found in database. Check ID and try Fetch/Extract again.`,
        });
      }
    } catch (error) {
      console.error("API Fetch Error:", error);
      setMessage({
        type: "error",
        text: `Error fetching DB data: ${
          error.response?.statusText || error.message
        }`,
      });
    } finally {
      setIsLoading(false);
    }
  }, []);

  // --- OCR Extraction Handler (POST) ---
  const processMasterFileForExtraction = async (masterFile) => {
    setIsLoading(true);
    setFetchedData(null);

    setMessage({ type: "info", text: "Processing document for metadata..." });

    const formData = new FormData();
    formData.append("file", masterFile);

    try {
      // 1. Send file to backend for OCR/Extraction
      const response = await axios.post(OCR_API_URL, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      console.log("OCR Response:", response.data);

      if (response.data.success && response.data.extractedData) {
        const extracted = response.data.extractedData;

        // 2. Populate form with extracted data
        const extractedForm = {
          paperId: extracted.paperId || "",
          examName: extracted.examName || "",
          className: extracted.className || "",
          subjectName: extracted.subjectName || "",
          examDate: extracted.examDate || "",
          totalMarks: extracted.totalMarks || "",
        };
        setForm(extractedForm);

        // 3. If ID is found, automatically trigger the DB fetch
        if (extracted.paperId) {
          setMessage({
            type: "info",
            text: `Metadata extracted. Auto-fetching content for ${extracted.paperId}...`,
          });
          await handleFetchPaper(extracted.paperId);
        } else {
          setMessage({
            type: "warning",
            text: "Metadata extracted, but Paper ID is missing. Please enter manually.",
          });
        }
      } else {
        setMessage({
          type: "error",
          text: "OCR failed or returned an invalid structure.",
        });
      }
    } catch (error) {
      console.error("OCR API Error:", error);
      setMessage({
        type: "error",
        text: `Failed to process file: ${error.message}. (Is the OCR backend running?)`,
      });
    } finally {
      setIsLoading(false);
    }
  };

  // --- CONSOLIDATED FILE HANDLING FOR INPUT FIELD ---
  const onFilesChange = (e) => {
    const uploadedFiles = Array.from(e.target.files || []);

    if (!uploadedFiles || uploadedFiles.length === 0) return;

    // If fetchedData exists, user is uploading the OMR Bundle.
    if (fetchedData) {
      setFiles(uploadedFiles);
    } else {
      // If fetchedData does NOT exist, this is the Master Document.
      // We only care about the files for processing, and clear any existing OMR data.
      setFiles(uploadedFiles.slice(0, 1));
    }
  };

  // --- Consolidated Check & Save Logic (THE SINGLE BUTTON HANDLER) ---
  const handleCheckPaper = async () => {
    if (isLoading) return;

    const uploadedCount = files.length;

    // SCENARIO 1: Need to Extract/Fetch (Master Doc uploaded OR ID manually entered)
    if (!fetchedData) {
      if (uploadedCount > 0) {
        // Master Document uploaded, run extraction
        await processMasterFileForExtraction(files[0]);
      } else if (form.paperId) {
        // ID manually entered, run direct fetch
        await handleFetchPaper(form.paperId);
      } else {
        setMessage({
          type: "error",
          text: "Please upload a Master Document or enter details manually.",
        });
      }
      return; // Wait for the async process to complete and update state
    }

    // SCENARIO 2: Ready for OMR processing (Data fetched, OMR sheets uploaded)
    if (fetchedData && uploadedCount > 0) {
      // --- SIMULATE OMR PROCESSING ---
      const updatedPaper = {
        status: "checked",
        fileCount: uploadedCount,
        paperId: form.paperId,
      };

      setMessage({
        type: "success",
        text: `[Simulated] OMR Bundle checked and results processed!`,
      });

      // Reset file input state for next operation
      setFiles([]);
      if (fileInputRef.current) fileInputRef.current.value = "";

      setTimeout(() => {
        setMessage(null);
        if (typeof onSaved === "function") onSaved(updatedPaper);
      }, 1000);
    } else if (fetchedData && uploadedCount === 0) {
      // SCENARIO 3: Data fetched, but no OMR sheets selected yet.
      setMessage({ type: "error", text: "Please upload the OMR sheets now." });
    }
  };

  // --- Print/Download Handlers ---

  const handleDownload = () => {
    if (!fetchedData) return;

    const { exam_name, paper_id, paper_questions, paper_answers } = fetchedData;

    const content =
      `--- ${exam_name} (${paper_id}) ---\n\n` +
      `METADATA:\nExam: ${form.examName}\nClass: ${form.className}\nMarks: ${form.totalMarks}\nDate: ${form.examDate}\n\n` +
      `QUESTION PAPER:\n${"=".repeat(20)}\n${paper_questions}\n\n` +
      `ANSWER KEY:\n${"=".repeat(20)}\n${paper_answers}\n`;

    const blob = new Blob([content], { type: "text/plain;charset=utf-8" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `${paper_id}_Full_Details.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    setMessage({
      type: "success",
      text: "Paper content downloaded successfully.",
    });
    setTimeout(() => setMessage(null), 2000);
  };

  const handlePrint = () => {
    if (!fetchedData) return;

    const printContents = document.getElementById(
      "printable-content-area"
    ).innerHTML;
    const originalContents = document.body.innerHTML;

    document.body.innerHTML = printContents;

    try {
      window.print();
    } catch (e) {
      console.error("Native printing failed, saving fallback:", e);
      // Sticking to your required localStorage for print fallback.
      localStorage.setItem(
        `print_fallback_${fetchedData.paper_id}`,
        JSON.stringify(fetchedData)
      );
      setMessage({
        type: "error",
        text: "Printing failed! Content saved to browser storage.",
      });
    } finally {
      document.body.innerHTML = originalContents;
      window.location.reload();
    }
  };

  // -------------------------------------------------
  // Render Helpers
  // -------------------------------------------------

  // Dynamic button text based on current workflow step
  const getButtonText = () => {
    if (isLoading) return "Processing...";
    if (!fetchedData && files.length > 0)
      return "Extract Metadata & Fetch Paper 🔎";
    if (!fetchedData && form.paperId) return "Fetch Details (Manual ID) 🔎";
    if (fetchedData && files.length === 0) return "Upload from file";
    if (fetchedData && files.length > 0)
      return `✅ Check & Save (${files.length} files)`;
    return "Check & Save"; // Default
  };

  return (
    <div className="w-full min-h-[80vh] p-6">
      <button
        onClick={onBack}
        className="mb-6 bg-slate-100 text-slate-700 px-4 py-2 rounded-md hover:bg-slate-200"
      >
        ⬅ Back to History
      </button>

      <div className="bg-white rounded-xl p-8 lg:p-12 shadow-sm border border-slate-200 w-full max-w-3xl mx-auto">
        <h1 className="text-2xl lg:text-3xl font-bold mb-6 text-slate-900 text-center">
          Test Page
        </h1>

        <div className="space-y-6">
          {/* FILE UPLOAD INPUT (Master Document OR OMR Bundle) */}
          <div className="border-b pb-6">
            <label className="block text-slate-700 font-medium mb-2">
              Upload OMR Bundle
            </label>
            <input
              id="omr-file-input"
              ref={fileInputRef}
              type="file"
              // Allow multiple files only if the paper details have been fetched (OMR Bundle)
              multiple={!!fetchedData}
              onChange={onFilesChange}
              accept={fetchedData ? "image/*,.pdf" : ".pdf,image/*,.doc,.docx"}
              className="w-full file:text-sm file:py-2 file:px-3 file:rounded file:border-0 file:bg-slate-100 file:cursor-pointer"
              disabled={isLoading}
            />

            <p className="text-sm text-slate-600 mt-2">
              {fetchedData
                ? `Status: Ready for OMR Bundle Upload (${files.length} selected)`
                : `Status: Please upload Master Document (or enter details manually below).`}
            </p>
            {files.length > 0 && (
              <div className="mt-2 text-sm text-slate-600">
                {files.length} {fetchedData ? "OMR" : "Master"} file(s)
                selected:
                <ul className="list-disc pl-5 mt-1">
                  {files.map((f, idx) => (
                    <li key={idx} className="truncate">
                      {f.name}
                    </li>
                  ))}
                </ul>
              </div>
            )}
          </div>

          {/* Metadata Display (EDITABLE / MANUAL INPUT) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InputField
              label="Paper ID (Editable)"
              value={form.paperId}
              onChange={handleFormChange}
              name="paperId"
              disabled={isLoading}
            />
            <InputField
              label="Exam Name"
              value={form.examName}
              onChange={handleFormChange}
              name="examName"
              disabled={isLoading}
            />
            <InputField
              label="Class"
              value={form.className}
              onChange={handleFormChange}
              name="className"
              disabled={isLoading}
            />
            <InputField
              label="Subject"
              value={form.subjectName}
              onChange={handleFormChange}
              name="subjectName"
              disabled={isLoading}
            />
            <InputField
              label="Exam Date"
              value={form.examDate}
              onChange={handleFormChange}
              name="examDate"
              disabled={isLoading}
            />
            <InputField
              label="Total Marks"
              value={form.totalMarks}
              onChange={handleFormChange}
              name="totalMarks"
              disabled={isLoading}
            />
          </div>

          {/* Single Action Button (Controls Extraction, Fetch, and OMR Check) */}
          <button
            onClick={handleCheckPaper}
            disabled={isLoading || (files.length === 0 && !form.paperId)}
            className="w-full bg-blue-600 text-white font-semibold py-3 rounded-lg hover:bg-blue-700 transition-all disabled:bg-blue-300"
          >
            {getButtonText()}
          </button>

          {/* Message Display (Original UI Structure) */}
          {message && (
            <div
              className={`mt-3 px-4 py-2 rounded-md text-sm font-medium ${
                message.type === "success"
                  ? "bg-green-50 text-green-700 border border-green-100"
                  : message.type === "info"
                  ? "bg-blue-50 text-blue-700 border border-blue-100"
                  : "bg-red-50 text-red-700 border border-red-100"
              }`}
            >
              {message.text}
            </div>
          )}
        </div>
      </div>

      {/* ---------------------------------------------------------------------- */}
      {/* ADDED SECTION: Fetched Paper Details & Print/Download */}
      {/* ---------------------------------------------------------------------- */}
      {fetchedData && (
        <div className="mt-8 bg-white rounded-xl p-8 lg:p-12 shadow-md border border-slate-300 w-full max-w-3xl mx-auto">
          <h2 className="text-xl font-bold mb-4 text-slate-900 text-center">
            Retrieved Paper Content
          </h2>

          <div
            id="printable-content-area"
            className="prose max-w-none text-sm p-4 border rounded-lg bg-gray-50 mb-4"
          >
            <h3 className="text-lg font-bold">
              Paper: {fetchedData.exam_name} ({fetchedData.paper_id})
            </h3>

            <h4 className="mt-4 mb-2 font-semibold text-base">
              Question Paper:
            </h4>
            <pre className="whitespace-pre-wrap text-slate-800 border-l-4 border-l-teal-400 pl-3 py-1 bg-white">
              {fetchedData.paper_questions}
            </pre>

            <h4 className="mt-4 mb-2 font-semibold text-base">Answer Key:</h4>
            <pre className="whitespace-pre-wrap text-slate-800 border-l-4 border-l-red-400 pl-3 py-1 bg-white">
              {fetchedData.paper_answers}
            </pre>
          </div>

          <div className="flex gap-4 mt-6">
            <button
              onClick={handlePrint}
              className="flex-1 bg-teal-600 text-white font-semibold py-3 rounded-lg hover:bg-teal-700 transition-all"
            >
              🖨️ Print Paper
            </button>
            <button
              onClick={handleDownload}
              className="flex-1 bg-yellow-600 text-white font-semibold py-3 rounded-lg hover:bg-yellow-700 transition-all"
            >
              ⬇️ Download as Text File
            </button>
          </div>
        </div>
      )}
    </div>
  );
};

export default OmrIntegratedPage;
